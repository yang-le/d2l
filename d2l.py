...
def show_heatmaps(matrices, xlabel, ylabel, titles=None, figsize=(2.5, 2.5), cmap='Reds'):
    """显示矩阵热图"""
    use_svg_display()
    num_rows, num_cols = matrices.shape[0].value, matrices.shape[1].value
...
def masked_softmax(X, valid_lens):
    ...
    if valid_lens is None:
        ...
        if len(valid_lens.shape) == 1:
            valid_len = tf.repeat(valid_lens, repeats=shape[1])
        else:
            valid_len = tf.reshape(valid_lens, shape=(-1,))
        # 最后一轴上被掩蔽的元素使用一个非常大的负值替换，从而其softmax输出为0
        X = sequence_mask(tf.reshape(X, shape=(-1, shape[-1])), valid_len, value=-1e6)
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def __init__(self, key_size, query_size, value_size, num_hiddens, num_heads, dropout, use_bias=False, **kwargs):
        ...
        self.W_q = tf.keras.layers.Dense(num_hiddens, use_bias=use_bias)
        self.W_k = tf.keras.layers.Dense(num_hiddens, use_bias=use_bias)
        self.W_v = tf.keras.layers.Dense(num_hiddens, use_bias=use_bias)
        self.W_o = tf.keras.layers.Dense(num_hiddens, use_bias=use_bias)
...
class EncoderBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, key_size, query_size, value_size, num_hiddens, norm_shape, ffn_num_hiddens, num_heads, dropout,
...
class TransformerEncoder(Encoder):
    ...
    def __init__(self, vocab_size, key_size, query_size, value_size, num_hiddens, norm_shape, ffn_num_hiddens,
        ...
        self.blks = [EncoderBlock(
            key_size, query_size, value_size, num_hiddens, norm_shape, ffn_num_hiddens, num_heads, dropout, use_bias)
...
class TransformerDecoder(AttentionDecoder):
    def __init__(self, vocab_size, key_size, query_size, value_size, num_hiddens, norm_shape, ffn_num_hiddens,
        ...
        self.blks = [DecoderBlock(
            key_size, query_size, value_size, num_hiddens, norm_shape, ffn_num_hiddens, num_heads, dropout, i)
    ...
    def call(self, X, state, **kwargs):
        X = self.pos_encoding(self.embedding(X) * tf.math.sqrt(tf.cast(self.num_layers, dtype=tf.float32)), **kwargs)
